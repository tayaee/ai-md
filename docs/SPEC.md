...
`.ai.md` 파일 하나 = 미니 앱 하나. 파일 경로가 URL prefix가 된다.

| 요청 | 동작 |
|---|---|
| `GET /` | `302 → /index.ai.md` |
| `GET /index.ai.md` | `dist/index.ai.md.html` 반환 (SPA) |
| `GET /convert.ai.md` | `302 → /convert.ai.md/docs` (Swagger UI) |
| `POST /convert.ai.md/convert` | 명세가 정의한 실제 엔드포인트 |
| 그 외 (`src/`에 대응 파일 없음) | `404` |
...
```
[브라우저] → :8080 nginx ──(dist/*.html 있으면 try_files 정적 서빙)
                     └─(그 외 전부)→ engine:8000 (호스트 비바인딩)
...
- Nginx는 멍청하다: `try_files $uri.html @engine` + 프록시 폴백만 한다.
...
| `AIMD_MAX_TOKENS` | `200000` | 출력 토큰 상한 |
...
- `src/index.ai.md`: 터미널 다크 테마(검정 배경/녹색 텍스트) 테트리스 SPA.
